...
function getQuiz(id, length) { // Takes length of quiz and category ID
  ...
  var easy = "easy";
  var query = db + "amount=" + length + "&category=" + id + "&type=multiple" + "&difficulty=" + easy;
  var fetchInit = {
...
function newQuiz(questions) {
  ...
  // Timer not implemented in this version
  ...
  $("#nextBtn").click(function() {
    ...
    if (userChoice == answerIndex) { // If users choice is correct...
      ...
      score++;
        ...
        answers[userChoice].classList.add("incorrectAnswer"); // Highlight user's choice red
        answers[answerIndex].classList.add("correctAnswer"); // Highlight correct answer green
    }
    count++; // Increment question counter
    setTimeout(function() { // Load next question after short interval
