...
//    List<Treasure> findActiveTreasure();
    ...
    //활성화 되어있는 보물 가져오기
    @Query
    Optional<Treasure> findByTreasureInventory_RunningTime_Status_Active();
    ...
    @Query("select  t.treasureInventory.targetPointList.targetPointId from Treasure t where t.treasureId =?1 and " +
            " t.treasureInventory.targetPointList.distinguish = 'FAKE' and " +
            " t.treasureInventory.targetPointList.targetPointId not in :ids ")
    List<Long> findTargetPointIds(Long treasureId, @Param("ids") List<Long> ids);

    //해당 보물의 해당 아이디들이 아니고 가장 난이도가 낮은 미션 아이디 하나 가져오는 메소드
    @Query("select t.treasureInventory.missionList.missionId from Treasure t where t.treasureId = ?1 and t.treasureInventory.missionList.missionId not in :ids")
    Mission findMission(Long treasureId, @Param("ids") List<Long> ids);
